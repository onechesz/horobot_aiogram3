...
async def horoscope_tomorrow_m(message: Message, session_maker: sessionmaker):
    ...
    async with session_maker() as session:
        async with session.begin():
            result: ScalarResult = await session.execute(select(Horoscope.horoscope_tomorrow)
...
def register(dispatcher: Dispatcher):
    dispatcher.message.register(horoscope_tomorrow_m, F.text == 'Гороскоп на завтра', state=None)
