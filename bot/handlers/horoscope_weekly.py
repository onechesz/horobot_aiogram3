...
async def horoscope_weekly_m(message: Message, session_maker: sessionmaker):
    ...
    async with session_maker() as session:
        async with session.begin():
            result: ScalarResult = await session.execute(select(Horoscope.horoscope_weekly)
...
def register(dispatcher: Dispatcher):
    dispatcher.message.register(horoscope_weekly_m, F.text == 'Гороскоп на неделю', state=None)
